    ...
    public class CHSock : DEVICE
    {
        ...
        */
        public class CONFIG : DEVICE_CONFIG
        {
            ...
            // Tx Timeout
            public int txTimeout = 0;
            // Rx Timeout
            public int rxTimeout = 0;
            // Port number
        ...
        */
        override public int Open(DEVICE_CONFIG config)
        {
            ...
            try
            {
                ...
                //g.thUser = new Thread(new ThreadStart(ThReceiving));
                //g.thUser.Start();
            ...
            catch (SocketException se) // Socket Exception //
            {
                switch (se.ErrorCode)
                {
                    case 10004: //////////////// interrupt //
                        Trace.WriteLine(
                            "CHSock.Receiving().SockErr = " 
                            + se.Message);
                        break;

                    case 10060: ////////////////// timeout //
                        Trace.WriteLine("CHSock.Receving.SockErr = " + se.Message);
                        g.err = new Exception("Timeout-Rx");
